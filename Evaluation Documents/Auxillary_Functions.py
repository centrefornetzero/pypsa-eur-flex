...
#Sankey diagram showing where the hydrogen and biomass buses end up (only one step flow calculated)
def Sankey_biomass_hydrogen(network_choice):
    ...
    Sankey_data['link_dispatch_from_source MWh'] = Sankey_data['link name'].map(dispatch_series) 
    ...
    negative_dispatch = Sankey_data[Sankey_data['link_dispatch_from_source MWh'] < 0]
    ...
    agg_flows = Sankey_data.groupby(['aggregate source', 'aggregate targets']).agg({
        'link_dispatch_from_source MWh': 'sum',
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            ...
            value=agg_flows['link_dispatch_from_source MWh'],
...
#bar graphs that show the change in technology thorugh the yeras across multiple networks
