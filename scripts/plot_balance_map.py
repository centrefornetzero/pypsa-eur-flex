...
if __name__ == "__main__":
    ...
    existing_components = components.intersection(
        eb.index.get_level_values("component").unique()
    )
    if len(existing_components) and len(carriers_in_eb):
        mask = eb.index.get_level_values("component").isin(existing_components) & (
            eb.index.get_level_values("carrier").isin(carriers_in_eb)
        )
        eb = eb.loc[~mask]
    ...
    if available_buses.empty:
        logger.warning(
            "No marginal prices available for carrier %s. Using zero price map fallback.",
            carrier,
        )
        regions["price"] = 0.0
        shift = 0
        ...
        prices = weights @ n.buses_t.marginal_price[available_buses] / weights.sum()
